<?
	...
	$mysqli = new mysqli("localhost", "root", "xxxx", "maletaboda" );
	
	/* If we are asked to delete a row */
	if( isset($type) && ($type=="del") && isset($val) ) {
		$res = $mysqli->query("DELETE FROM nerea WHERE id='".$val."'");
	}
	
	/* If we are asked to add a row */
	if( isset($type) && ($type=="add") && isset($val) ) {
		$res = $mysqli->query("INSERT INTO nerea (item) VALUES ('".$val."')" );
	}
	
	/* Show item data */
	$res = $mysqli->query("SELECT * FROM nerea" );
	printf( "[" );
